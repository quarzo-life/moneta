...
describe("decimal factors", () => {
  test("does not round with a positive integer quotient", () => {
    expect(halfAwayFromZero(20n, 10n)).toEqual(2n);
  });
  ...
  test("does not round with a zero quotient", () => {
    expect(halfAwayFromZero(0n, 10n)).toEqual(0n);
  });

  test(
    "rounds to the nearest integer away from zero with a positive half quotient",
    () => {
      expect(halfAwayFromZero(15n, 10n)).toEqual(2n);
    },
  );
  ...
  test(
    ...
    () => {
      expect(halfAwayFromZero(-25n, 10n)).toEqual(-3n);
    },
  );
  ...
  test("rounds up with any positive float quotient above half", () => {
    fc.assert(
      fc.property(fc.integer({ min: 6, max: 9 }), (a) => {
        expect(halfAwayFromZero(BigInt(a), 10n)).toEqual(1n);
      }),
    );
  });
    ...
    () => {
      fc.assert(
        fc.property(fc.integer({ min: 1, max: 4 }), (a) => {
          expect(halfAwayFromZero(BigInt(a), 10n)).toEqual(0n);
        }),
      );
    },
  );
...
describe("non-decimal factors", () => {
  test("does not round with a positive integer quotient", () => {
    expect(halfAwayFromZero(20n, 5n)).toEqual(4n);
  });
  ...
  test("does not round with a zero quotient", () => {
    expect(halfAwayFromZero(0n, 5n)).toEqual(0n);
  });

  test(
    "rounds to the nearest integer away from zero with a positive half quotient",
    () => {
      expect(halfAwayFromZero(3n, 2n)).toEqual(2n);
    },
  );
  ...
  test(
    "rounds to the nearest integer away from zero with a negative half quotient",
    () => {
      expect(halfAwayFromZero(-5n, 2n)).toEqual(-3n);
    },
  );
  ...
  test("rounds up with any positive float quotient above half", () => {
    fc.assert(
      fc.property(fc.integer({ min: 3, max: 4 }), (a) => {
        expect(halfAwayFromZero(BigInt(a), 5n)).toEqual(1n);
      }),
    );
  });
    ...
    () => {
      fc.assert(
        fc.property(fc.integer({ min: 1, max: 2 }), (a) => {
          expect(halfAwayFromZero(BigInt(a), 5n)).toEqual(0n);
        }),
      );
    },
  );
