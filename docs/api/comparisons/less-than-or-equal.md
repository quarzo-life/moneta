...
description: Check whether the value of a Money object is less than or equal to another.
returns: boolean
---
...
```ts
import { lessThanOrEqual, money, USD } from "jsr:@quarzo-life/moneta";

const first = money({ amount: 500n, currency: USD });
const second = money({ amount: 500n, currency: USD });

lessThanOrEqual(first, second); // true
```
